...
public struct Secp256k1PublicKey: SuiPublicKeyProtocol {
    private let key: P256K.Signing.PublicKey

    public init(rawBytes: [UInt8]) throws {
        guard rawBytes.count == 33 else {
            throw Secp256k1Error.invalidPublicKeyLength
        }
        self.key = try P256K.Signing.PublicKey(dataRepresentation: Data(rawBytes), format: .compressed)
    }

    init(_ key: P256K.Signing.PublicKey) {
        self.key = key
    }

    public var rawBytes: [UInt8] {
        [UInt8](key.dataRepresentation)
    }

    public func toSuiBytes() -> [UInt8] {
        SuiPublicKeyEncoding.toSuiBytes(rawPublicKey: rawBytes, scheme: .secp256k1)
    }

    public func toSuiPublicKey() -> String {
        SuiPublicKeyEncoding.toSuiPublicKeyBase64(rawPublicKey: rawBytes, scheme: .secp256k1)
    }

    public func toSuiAddress() -> String {
        SuiPublicKeyEncoding.toSuiAddress(rawPublicKey: rawBytes, scheme: .secp256k1)
    }
    ...
    public func verify(message: [UInt8], signature: [UInt8]) -> Bool {
        ...
        guard let sig = try? P256K.Signing.ECDSASignature(dataRepresentation: Data(signature)) else {
            return false
        }
    ...
    public func verifySerialized(message: [UInt8], serializedSignature: String) throws -> Bool {
        ...
        guard parsed.scheme == .secp256k1 else {
            throw Secp256k1Error.invalidSerializedSignature
        }
...
public struct Secp256k1Keypair {
    ...
    public init() throws {
        self.privateKey = try P256K.Signing.PrivateKey()
    }
    ...
    public init(privateKeyBytes: [UInt8]) throws {
        ...
        self.privateKey = try P256K.Signing.PrivateKey(dataRepresentation: Data(privateKeyBytes))
    }

    public init(secretKey: String) throws {
        let parsed = try SuiPrivateKey.decode(secretKey)
        guard parsed.scheme == .secp256k1 else {
            throw Secp256k1Error.invalidSecretKeyScheme
        }
        try self.init(privateKeyBytes: parsed.secretKey)
    }

    public var publicKey: Secp256k1PublicKey {
        Secp256k1PublicKey(privateKey.publicKey)
    }

    public var privateKeyBytes: [UInt8] {
        [UInt8](privateKey.dataRepresentation)
    }
    ...
    public func getSecretKey() throws -> String {
        try SuiPrivateKey.encode(secretKey: privateKeyBytes, scheme: .secp256k1)
    }
    ...
    public func toSerializedSignature(message: [UInt8]) throws -> String {
        ...
        return try SerializedSignature.toBase64(scheme: .secp256k1, signature: signature, publicKey: publicKey.rawBytes)
    }
    ...
    public static func verifyWithPublicKey(publicKey: [UInt8], message: [UInt8], signature: [UInt8]) -> Bool {
        guard let key = try? Secp256k1PublicKey(rawBytes: publicKey) else {
            return false
        }
...
extension Secp256k1Keypair: SuiSigner {}
