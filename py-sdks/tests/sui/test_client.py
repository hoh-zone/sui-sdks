...
class TestSuiClient(unittest.TestCase):
    def test_discover_rpc_api(self):
    ...
    def test_get_object_and_get_objects(self):
        ...
        c.multi_get_objects(["0x2", "0x3"], {"showType": True})
    ...
    def test_coin_helpers(self):
        mock = _MockJsonRpcClient()
        c = SuiClient(endpoint="https://example.invalid", _rpc_client=mock)

        c.get_all_coins(owner="0xabc", cursor="next", limit=10)
        c.get_balance(owner="0xabc")
        c.get_all_balances(owner="0xabc")
        c.get_coin_metadata("0x2::sui::SUI")
        c.get_total_supply("0x2::sui::SUI")
    ...
    def test_object_and_system_helpers(self):
        mock = _MockJsonRpcClient()
        c = SuiClient(endpoint="https://example.invalid", _rpc_client=mock)

        c.get_owned_objects("0xabc", {"filter": {"MatchAll": []}}, "cur", 5)
        c.get_owned_objects_legacy("0xabc", {"filter": {"MatchAll": []}}, "cur", 5)
        c.get_dynamic_fields("0xparent", "cur2", 7)
        c.get_dynamic_field_object("0xparent", {"type": "address", "value": "0xabc"})
        c.get_latest_sui_system_state()
        c.get_reference_gas_price()
    ...
    def test_transaction_helpers(self):
        ...
        c.query_transaction_blocks(query, cursor="cursor", limit=20, descending_order=True)
        c.get_transaction_block("digest", {"showEvents": True})
    ...
    def test_past_objects_and_move_helpers(self):
        mock = _MockJsonRpcClient()
        c = SuiClient(endpoint="https://example.invalid", _rpc_client=mock)
        past_objects = [{"objectId": "0x1", "version": 12}, {"objectId": "0x2", "version": 99}]

        c.try_get_past_object("0x1", 12, {"showContent": True})
        c.try_multi_get_past_objects(past_objects, {"showType": True})
        c.get_normalized_move_modules_by_package("0x2")
        c.get_normalized_move_module("0x2", "coin")
        c.get_normalized_move_function("0x2", "coin", "balance")
        c.get_move_function_arg_types("0x2", "coin", "balance")
        c.get_normalized_move_struct("0x2", "coin", "Coin")
    ...
    def test_network_and_checkpoint_helpers(self):
        mock = _MockJsonRpcClient()
        c = SuiClient(endpoint="https://example.invalid", _rpc_client=mock)

        c.get_events_by_transaction("0xdigest")
        c.get_checkpoint("100")
        c.get_checkpoints(cursor="99", limit=10, descending_order=True)
        c.get_committee_info(epoch="3")
        c.get_protocol_config(version="6")
        c.get_chain_identifier()
        c.multi_get_transaction_blocks(["d1", "d2"], {"showEffects": True})
        c.get_total_transaction_blocks()
    ...
    def test_name_service_and_validators_helpers(self):
        mock = _MockJsonRpcClient()
        c = SuiClient(endpoint="https://example.invalid", _rpc_client=mock)

        c.resolve_name_service_address("alice.sui")
        c.resolve_name_service_names("0xabc", cursor="c1", limit=5)
        c.get_validators_apy()
        c.get_stakes("0xabc")
        c.get_stakes_by_ids(["0xstake1", "0xstake2"])
    ...
    def test_iter_helpers(self):
        ...
        checkpoints = list(c.iter_checkpoints(limit=1))
        events = list(c.iter_events(query={"All": []}, limit=1))
        checkpoints_limited = list(c.iter_checkpoints(limit=1, max_items=1))
        events_limited = list(c.iter_events(query={"All": []}, limit=1, max_items=1))
    ...
    def test_iter_owned_objects_and_transactions(self):
        class _PagerMock:
            def __init__(self):
                self.calls = []
        ...
        owned = list(c.iter_owned_objects(owner="0xabc", limit=1))
        txs = list(c.iter_transaction_blocks(query={"FromAddress": "0xabc"}, limit=1))
        coins = list(c.iter_all_coins(owner="0xabc", limit=1))
        fields = list(c.iter_dynamic_fields(parent_object_id="0xparent", limit=1))
        owned_limited = list(c.iter_owned_objects(owner="0xabc", limit=1, max_items=1))
        txs_limited = list(c.iter_transaction_blocks(query={"FromAddress": "0xabc"}, limit=1, max_items=1))
