...
public final class Secp256r1Keypair implements Keypair {
    private final PrivateKey privateKey;
    private final PublicKey publicKey;

    private Secp256r1Keypair(PrivateKey privateKey, PublicKey publicKey) {
    ...
    public static Secp256r1Keypair generate() {
        try {
            ...
            return new Secp256r1Keypair(kp.getPrivate(), kp.getPublic());
        } catch (Exception e) {
            throw new IllegalStateException("Failed to generate secp256r1 keypair", e);
        }
    }
    ...
    public byte[] sign(byte[] message) {
        try {
            ...
            throw new IllegalStateException("Failed to sign with secp256r1", e);
        }
    }
    ...
    public static boolean verifyWithPublicKey(byte[] compressedPublicKey, byte[] message, byte[] signature) {
        try {
            Signature s = Signature.getInstance("SHA256withECDSA", "BC");
            s.initVerify(publicKeyFromCompressed(compressedPublicKey, "secp256r1"));
            s.update(message);
