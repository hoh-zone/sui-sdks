use fastcrypto::secp256r1::{
    Secp256r1KeyPair, Secp256r1PrivateKey, Secp256r1Signature, SECP256R1_PRIVATE_KEY_LENGTH,
};
use fastcrypto::traits::{KeyPair, Signer, ToFromBytes, VerifyingKey};

use crate::crypto;

const SECP256R1_SCHEME_FLAG: u8 = 0x02;
...
pub enum KeypairError {
    ...
    InvalidPrivateKey,
    #[error("invalid Sui private key scheme flag: expected 0x02, got 0x{0:02x}")]
    InvalidScheme(u8),
}

pub struct Keypair {
    inner: Secp256r1KeyPair,
}

impl Keypair {
    pub fn generate() -> Self {
        let mut rng = rand::thread_rng();
        let inner = Secp256r1KeyPair::generate(&mut rng);
        Self { inner }
    }

    pub fn from_secret_key(secret_key: &[u8]) -> Result<Self, KeypairError> {
        if secret_key.len() != SECP256R1_PRIVATE_KEY_LENGTH {
            return Err(KeypairError::InvalidPrivateKeyLength {
                expected: SECP256R1_PRIVATE_KEY_LENGTH,
                actual: secret_key.len(),
            });
        }
        let private = Secp256r1PrivateKey::from_bytes(secret_key).map_err(|_| KeypairError::InvalidPrivateKey)?;
        Ok(Self {
            inner: private.into(),
        })
    }
    ...
    pub fn from_sui_private_key(encoded: &str) -> Result<Self, KeypairError> {
        ...
        if bytes[0] != SECP256R1_SCHEME_FLAG {
    ...
    pub fn verify(&self, message: &[u8], signature: &[u8]) -> Result<bool, KeypairError> {
        let sig = Secp256r1Signature::from_bytes(signature).map_err(|_| KeypairError::InvalidSignature)?;
    ...
    pub fn to_sui_address(&self) -> String {
        crypto::to_sui_address(SECP256R1_SCHEME_FLAG, self.inner.public().as_ref())
    }

    pub fn to_sui_private_key(&self) -> String {
        let mut bytes = Vec::with_capacity(1 + SECP256R1_PRIVATE_KEY_LENGTH);
        bytes.push(SECP256R1_SCHEME_FLAG);
