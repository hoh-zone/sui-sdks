...
impl Client {
    fn validate_package_id_or_named_package(package_id: &str) -> bool {
        utils::is_valid_sui_object_id(package_id) || registry::is_valid_named_package(package_id)
    }
    ...
    pub async fn get_normalized_move_modules_by_package(
        ...
        if !Self::validate_package_id_or_named_package(package_id) {
            return Err(JsonRpcError::InvalidAddress);
        }
        self.call(
