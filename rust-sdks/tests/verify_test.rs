...
use sui_sdks_rust::sui::keypairs::{ed25519, secp256k1, secp256r1};
use sui_sdks_rust::sui::verify;
...
fn verify_secp_signatures() {
    ...
    let ok_k1 = verify::verify_signature(SignatureScheme::Secp256k1, &k1.public_key_bytes(), msg, &sig_k1)
        .expect("verify k1");
    let ok_r1 = verify::verify_signature(SignatureScheme::Secp256r1, &r1.public_key_bytes(), msg, &sig_r1)
        .expect("verify r1");
...
fn verify_transaction_intent() {
    let kp = ed25519::Keypair::generate();
    let tx = b"tx-bytes";
    let intent_msg = sui_sdks_rust::crypto::message_with_intent([0, 0, 0], tx);
