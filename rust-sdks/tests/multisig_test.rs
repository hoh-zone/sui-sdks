...
use sui_sdks_rust::crypto::SignatureScheme;
use sui_sdks_rust::sui::keypairs::ed25519;
use sui_sdks_rust::sui::multisig::{parse_multisig, serialize_multisig, MultiSigEntry, MultiSigPublicKey, MultiSigSerialized, WeightedPublicKey};
...
fn multisig_serialize_parse_and_verify() {
    ...
    let ms = MultiSigPublicKey {
        public_keys: vec![
            WeightedPublicKey { scheme: SignatureScheme::Ed25519, public_key: pk1.clone(), weight: 1 },
            WeightedPublicKey { scheme: SignatureScheme::Ed25519, public_key: pk2.clone(), weight: 1 },
        ],
        threshold: 2,
    };
    ...
    let serialized = serialize_multisig(&MultiSigSerialized {
        signatures: vec![
            MultiSigEntry { public_key: pk1, signature: sig1 },
            MultiSigEntry { public_key: pk2, signature: sig2 },
        ],
        bitmap: vec![0, 1],
        threshold: 2,
    })
