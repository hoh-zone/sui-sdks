...
use crate::deepbook_v3::encode::{
    encode_bool, encode_option_u64, encode_u8, encode_u64, encode_u128, encode_vec_u128,
};
...
impl<'a> DeepBookContract<'a> {
    ...
    pub fn modify_order(
        ...
        Ok(tx.move_call(
            ...
            vec![pool_obj, manager_obj, proof, order_id_arg, quantity_arg, clock_obj],
            vec![base.type_tag.clone(), quote.type_tag.clone()],
        ))
    }
    ...
    pub fn swap_exact_base_for_quote(
        ...
        Ok(tx.move_call(
            ...
            vec![pool_obj, base_coin_obj, deep_coin_obj, min_quote_arg, clock_obj],
            vec![base.type_tag.clone(), quote.type_tag.clone()],
        ))
    }
    ...
    pub fn swap_exact_quote_for_base(
        ...
        Ok(tx.move_call(
            ...
            vec![pool_obj, quote_coin_obj, deep_coin_obj, min_base_arg, clock_obj],
            vec![base.type_tag.clone(), quote.type_tag.clone()],
        ))
    }
...
impl<'a> MarginTPSLContract<'a> {
    ...
    pub fn new_condition(
        ...
        let quote = self.config.get_coin(&pool.quote_coin)?;
        let input_price =
            ((trigger_price * FLOAT_SCALAR * quote.scalar as f64) / base.scalar as f64).round() as u64;
        ...
        Ok(tx.move_call(
            &format!("{}::tpsl::new_condition", self.config.package_ids.margin_package_id),
    ...
    pub fn add_conditional_limit_order(
        ...
        let condition = self.new_condition(tx, &manager.pool_key, trigger_below_price, trigger_price)?;
...
impl<'a> PoolProxyContract<'a> {
    fn pool_proxy_target(&self, function: &str) -> String {
        format!("{}::pool_proxy::{function}", self.config.package_ids.margin_package_id)
    }
    ...
    pub fn cancel_orders(
        ...
        Ok(tx.move_call(
            ...
            vec![registry_obj, manager_obj, pool_obj, order_ids_arg, clock_obj],
            vec![base.type_tag.clone(), quote.type_tag.clone()],
        ))
    }
...
impl<'a> MarginLiquidationsContract<'a> {
    ...
    pub fn liquidate_base(
        ...
        let repay_amount_scaled = repay_amount.map(|v| (v * base_coin.scalar as f64).round() as u64);
    ...
    pub fn liquidate_quote(
        ...
        let base_margin_pool = self.config.get_margin_pool(&pool.base_coin)?;
        let quote_margin_pool = self.config.get_margin_pool(&pool.quote_coin)?;
        let repay_amount_scaled = repay_amount.map(|v| (v * quote_coin.scalar as f64).round() as u64);
...
impl<'a> MarginMaintainerContract<'a> {
    ...
    pub fn new_interest_config(
        ...
        let optimal_utilization_arg =
            tx.pure_bytes(&encode_u64((optimal_utilization * FLOAT_SCALAR).round() as u64));
    ...
    pub fn new_margin_pool_config(
        ...
        let coin = self.config.get_coin(coin_key)?;
        let supply_cap_arg = tx.pure_bytes(&encode_u64((supply_cap * coin.scalar as f64).round() as u64));
        let max_utilization_rate_arg =
            tx.pure_bytes(&encode_u64((max_utilization_rate * FLOAT_SCALAR).round() as u64));
        let referral_spread_arg =
            tx.pure_bytes(&encode_u64((referral_spread * FLOAT_SCALAR).round() as u64));
        let min_borrow_arg = tx.pure_bytes(&encode_u64((min_borrow * coin.scalar as f64).round() as u64));
        Ok(tx.move_call(
    ...
    pub fn new_margin_pool_config_with_rate_limit(
        ...
        let referral_spread_arg =
            tx.pure_bytes(&encode_u64((referral_spread * FLOAT_SCALAR).round() as u64));
        let min_borrow_arg = tx.pure_bytes(&encode_u64((min_borrow * coin.scalar as f64).round() as u64));
        let rate_limit_capacity_arg =
            tx.pure_bytes(&encode_u64((rate_limit_capacity * coin.scalar as f64).round() as u64));
    ...
    pub fn enable_deepbook_pool_for_loan(
        ...
        Ok(tx.move_call(
            ...
            vec![pool_obj, registry_obj, deepbook_pool_obj, cap_obj, clock_obj],
            vec![margin_pool.type_tag.clone()],
        ))
    }
    ...
    pub fn update_interest_params(
        ...
        Ok(tx.move_call(
            ...
            vec![pool_obj, registry_obj, interest_config_object, cap_obj, clock_obj],
            vec![margin_pool.type_tag.clone()],
        ))
    }
    ...
    pub fn update_margin_pool_config(
        ...
        Ok(tx.move_call(
            ...
            vec![pool_obj, registry_obj, margin_pool_config_object, cap_obj, clock_obj],
            vec![margin_pool.type_tag.clone()],
        ))
    }
}
...
impl<'a> DeepBookAdminContract<'a> {
    ...
    fn registry_target(&self, function: &str) -> String {
        format!("{}::registry::{function}", self.config.package_ids.deepbook_package_id)
    }
    ...
    pub fn adjust_tick_size(
        ...
        let adjusted_tick_size =
            ((new_tick_size * FLOAT_SCALAR * quote.scalar as f64) / base.scalar as f64).round() as u64;
        let pool_obj = tx.object(pool.address.clone());
        let tick_arg = tx.pure_bytes(&encode_u64(adjusted_tick_size));
        let cap_obj = tx.object(admin_cap_id.to_string());
        let clock_obj = tx.object("0x6");
...
impl<'a> MarginAdminContract<'a> {
    ...
    pub fn register_deepbook_pool(
        ...
        Ok(tx.move_call(
            ...
            vec![registry_obj, admin_cap_obj, pool_obj, pool_config_object, clock_obj],
            vec![base.type_tag.clone(), quote.type_tag.clone()],
        ))
    }
