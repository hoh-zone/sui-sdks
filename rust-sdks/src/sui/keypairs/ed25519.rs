...
pub enum KeypairError {
    ...
    InvalidPrivateKey,
    #[error("invalid Sui private key scheme flag: expected 0x00, got 0x{0:02x}")]
    InvalidScheme(u8),
}
...
impl Keypair {
    pub fn generate() -> Self {
        ...
        let inner = Ed25519KeyPair::generate(&mut rng);
        Self { inner }
    }

    pub fn from_secret_key(secret_key: &[u8]) -> Result<Self, KeypairError> {
        if secret_key.len() != ED25519_PRIVATE_KEY_LEN {
            return Err(KeypairError::InvalidPrivateKeyLength {
                expected: ED25519_PRIVATE_KEY_LEN,
                actual: secret_key.len(),
            });
        }
        let private = Ed25519PrivateKey::from_bytes(secret_key).map_err(|_| KeypairError::InvalidPrivateKey)?;
        Ok(Self {
            inner: private.into(),
        })
    }
    ...
    pub fn verify(&self, message: &[u8], signature: &[u8]) -> Result<bool, KeypairError> {
        let sig = Ed25519Signature::from_bytes(signature).map_err(|_| KeypairError::InvalidSignature)?;
    ...
    pub fn to_sui_address(&self) -> String {
        crypto::to_sui_address(ED25519_SCHEME_FLAG, self.inner.public().as_ref())
    }

    pub fn to_sui_private_key(&self) -> String {
        let mut bytes = Vec::with_capacity(1 + ED25519_PRIVATE_KEY_LEN);
